...
class FrequencyPositionalEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.num_freqs > 0:
            ...
            if frequencies.device != x.device or frequencies.dtype != x.dtype:
                frequencies = frequencies.to(device=x.device, dtype=x.dtype)
            embed = (x[..., None].contiguous() * frequencies).view(
                *x.shape[:-1], -1
            )
            if self.use_pmpe:
                phase_tensor = self.phase
                if phase_tensor.device != x.device or phase_tensor.dtype != x.dtype:
                    phase_tensor = phase_tensor.to(device=x.device, dtype=x.dtype)
                phase = (x[..., None].contiguous()*torch.pi*0.5 + phase_tensor).view(
                    *x.shape[:-1], -1
                )
